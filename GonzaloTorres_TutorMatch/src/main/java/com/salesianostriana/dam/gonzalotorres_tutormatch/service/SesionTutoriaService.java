...
public class SesionTutoriaService extends BaseServiceImpl<SesionTutoria, Long, SesionTutoriaRepository> {
    ...
    public SesionTutoria calcularCoste(SesionTutoria sesion) {
        ...
        sesion.setCosteTotal(Math.round(coste * 100.0) / 100.0);
        return sesion;
    }
 
    public SesionTutoria aplicarRecargoPorUrgencia(SesionTutoria sesion) {
        double costeConRecargo = java.util.stream.Stream.of(sesion)
                .filter(s -> java.time.Duration.between(
                        LocalDateTime.now(), s.getFechaInicio()).toHours() < 24)
                .mapToDouble(s -> s.getCosteTotal() * 1.20)
                .findFirst()
                .orElse(sesion.getCosteTotal());
 
        sesion.setCosteTotal(Math.round(costeConRecargo * 100.0) / 100.0);
        return sesion;
    }
    
    public SesionTutoria aplicarDescuentoVolumen(SesionTutoria sesion) {
        long totalSesiones = sesionTutoriaRepository.countByEstudianteAndTutor(
                sesion.getEstudiante().getId(), sesion.getTutor().getId());

        double costeConDescuento = java.util.stream.Stream.of(sesion)
                .filter(s -> totalSesiones > 3)
                .mapToDouble(s -> s.getCosteTotal() * 0.90)
                .findFirst()
                .orElse(sesion.getCosteTotal());

        sesion.setCosteTotal(Math.round(costeConDescuento * 100.0) / 100.0);
        return sesion;
    }
